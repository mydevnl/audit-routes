...
class FailedPercentageTest extends TestCase
{
    #[Test]
    public function it_calculates_the_failed_percentage_for_all_audited_routes(): void
    {
        $routes = ['1', '2', '3'];
        ...
        [$aggregator] = AuditRoutes::for($routes)
            ...
            ->aggregate(new FailedPercentage());

        $this->assertEquals(33.33, $aggregator->getResult());
    }
    ...
    public function it_does_not_take_ignored_routes_into_account(): void
    {
        ...
        $this->assertEquals(25, $aggregator->getResult());
    }

    #[Test]
    public function it_can_be_given_a_name(): void
    {
        $aggregator = new FailedPercentage('Unique name');
    ...
    {
        $aggregator = new FailedPercentage();

        $this->assertEquals('failed_percentage', $aggregator->getAggregator());
    }
    ...
    public function it_can_return_its_data(): void
    {
        ...
        [$aggregator] = AuditRoutes::for($routes)
            ->setBenchmark(2)
            ->run([DummyAuditor::class])
            ->aggregate(new FailedPercentage('Unique name'));

        $this->assertEquals([
            'aggregator' => 'failed_percentage',
            'name'       => 'Unique name',
            'result'     => 20,
        ], $aggregator->toArray());
    }
